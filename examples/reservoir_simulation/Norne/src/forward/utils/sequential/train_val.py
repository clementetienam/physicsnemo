...
=====================================================================
                    SEQUENTIAL TRAINING AND VALIDATION
=====================================================================

This module provides training and validation functions for sequential processing
in reservoir simulation forward modeling. It includes functions for model
training, validation, and performance evaluation.

Key Features:
- Sequential training step implementation
- Validation step implementation
- Model performance evaluation
- Loss computation and optimization

Usage:
    from forward.utils.sequential.train_val import (
        training_step_pl,
        validation_step_pl,
...
def training_step_pl(
    ...
    actnum = neededM["actnum"].expand(batch_size, -1, -1, -1, -1).clamp(min=1e-6)
    ...
    if "SGAS" in output_variables:
        outputsg = model(input_tensor, mode="gas")  # Use input_now
        gas_pred = outputsg["gas"]
        targets_gas = TARGETS["GAS"]
    if "SWAT" in output_variables:
        outputsw = model(input_tensor, mode="saturation")
        water_pred = outputsw["saturation"]
        target_saturation = TARGETS["SATURATION"]
    if "SOIL" in output_variables:
        outputso = model(input_tensor, mode="oil")
        oil_pred = outputso["oil"]
    ...
    if "SWAT" in output_variables:
        ...
        temp = normalize_tensors_adjusted2(
            water_pred - (1 - (target_gas["gas_sat"] + target_oil["oil_sat"]))
        )
        unie = torch.abs(actnum * temp).sum()
        loss += water_loss  # + unie
    if "SOIL" in output_variables:
        oil_loss = loss_func(
            oil_pred,
            target_oil["oil_sat"],
        ...
        temp = normalize_tensors_adjusted2(
            oil_pred - (1 - (target_gas["gas_sat"] + target_saturation["water_sat"]))
        )
        unie = torch.abs(actnum * temp).sum()
        loss += oil_loss  # +  unie
    if "SGAS" in output_variables:
        ...
        temp = normalize_tensors_adjusted2(
            gas_pred - (1 - (target_oil["oil_sat"] + target_saturation["water_sat"]))
        )
        unie = torch.abs(actnum * temp).sum()
        loss += gass_loss  # + unie
    inputs1 = {
...
# Function to apply decorators later
def setup_training_functions(composite_model, composite_optimizer, logger):
    """Apply decorators after models are properly initialized"""

    training_step = StaticCaptureTraining(
        model=composite_model,
        optim=composite_optimizer,
        logger=logger,
        use_amp=False,
        use_graphs=True,
    )(training_step_pl)
...
# Export the setup function instead of the decorated functions
__all__ = ["training_step_pl", "validation_step_pl", "setup_training_functions"]
