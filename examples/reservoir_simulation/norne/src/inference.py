...
def main(cfg: DictConfig) -> None:
    ...
    if not os.path.exists(to_absolute_path("../RESULTS/FORWARD_RESULTS")):
        os.makedirs(
            to_absolute_path("../RESULTS/FORWARD_RESULTS"), exist_ok=True
        )
    else:
...
Gaussian Process Experts. arXiv preprint arXiv:2006.13309, 2020.\n"
    ...
    active_mask_3d = np.genfromtxt(os.path.join(source_dir, "actnum.out"), dtype="float")
    ...
    msg = "Reservoir simulation with FLOW  took: {} secs (Wall clock time)".format(timedelta(
    ...
    msg = (
        f"Reservoir simulation with Nvidia PhysicsNeMo (CCR - Hard prediction)  took: {timedelta(seconds=round(elapsed_time_secs2))} secs (Wall clock time)"
    )
    logger.info(msg)
    ...
    compare_and_analyze_results(
        ...
        fields=CompareFields(
            pressure_pred=pressure, pressure_true=pressure_true,
            water_pred=Swater, water_true=Swater_true,
            oil_pred=Soil, oil_true=Soil_true,
            gas_pred=Sgas, gas_true=Sgas_true,
        ),
        wells=CompareWells(
            N_pr=N_pr, N_injw=N_injw, N_injg=N_injg,
            lenwels=lenwels,
        ...
        runtime=CompareRuntime(
            cfg=cfg, device=device, num_cores=num_cores,
            oldfolder=oldfolder, folderr=folderr,
        ...
        norms=CompareNorms(
            ...
            minP=minP, maxP=maxP,
            minQ=minQ, maxQ=maxQ,
            minQw=minQw, maxQw=maxQw,
            minQg=minQg, maxQg=maxQg,
        ),
        surrogate=CompareSurrogate(
            models=models, degg=degg, experts=experts, inn=inn,
        ),
        flow=CompareFlow(
            active_cells_ensemble=active_cells_ensemble,
            active_mask_3d=active_mask_3d,
            awater=awater, agas=agas, aoil=aoil, aqq=aqq,
            Time=Time,
        ),
    )
    os.chdir(oldfolder)
