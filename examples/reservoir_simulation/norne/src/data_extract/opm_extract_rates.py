...
def process_and_print(data_dict, dict_name):
    ...
    for key in data_dict:
        ...
        if _n_nan > 0:
            logger.warning("Replacing %d NaN values in %s[%s] with 1e-6", _n_nan, dict_name, key)
        if _n_inf > 0:
            logger.warning("Replacing %d Inf values in %s[%s] with 1e-6", _n_inf, dict_name, key)
